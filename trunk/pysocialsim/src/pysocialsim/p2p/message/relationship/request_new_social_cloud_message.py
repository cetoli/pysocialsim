...
class RequestNewSocialCloudMessage(AbstractP2PMessage):
    
    def __init__(self, id, sourceId, targetId, ttl, priority):
        self.initialize("REQUEST_NEW_SOCIAL_CLOUD", id, sourceId, targetId, ttl, priority)
    
    @public
    def clone(self):
        message = RequestNewSocialCloudMessage(self.getId(), self.getSourceId(), self.getTargetId(), self.getTTL(), self.getPriority())
