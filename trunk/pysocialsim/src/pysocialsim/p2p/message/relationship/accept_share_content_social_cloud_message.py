...
class AcceptShareContentSocialCloudMessage(AbstractP2PMessage):
    
    def __init__(self, id, sourceId, targetId, ttl, priority):
        self.initialize("ACCEPT_SHARE_CONTENT_SOCIAL_CLOUD", id, sourceId, targetId, ttl, priority)
    
    @public
    def clone(self):
        message = AcceptShareContentSocialCloudMessage(self.getId(), self.getSourceId(), self.getTargetId(), self.getTTL(), self.getPriority())
