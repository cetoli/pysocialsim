...
class CreateRelationshipMessage(AbstractP2PMessage):
    
    def __init__(self, id, sourceId, targetId, ttl, priority):
        self.initialize("CREATE_RELATIONSHIP", id, sourceId, targetId, ttl, priority)
    
    @public
    def clone(self):
        message = CreateRelationshipMessage(self.getId(), self.getSourceId(), self.getTargetId(), self.getTTL(), self.getPriority())
