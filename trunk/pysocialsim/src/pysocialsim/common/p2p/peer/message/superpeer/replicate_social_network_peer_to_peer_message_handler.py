...
class ReplicateSocialNetworkPeerToPeerMessageHandler(AbstractPeerToPeerMessageHandler):
    ...
    def execute(self):
        ...
        if peer.countNeighbors() > 0:
            neighbors = [n for n in peer.getNeighbors() if not n.getId() in message.getPeerIds()]
            for neighbor in neighbors:
                messageClone = message.clone()
                messageClone.init(message.getId(), peer.getId(), neighbor.getId(), message.getTTL(), message.getPriority(), 512, message.getTime())
