...
class AbstractPeerToPeerProtocol(Object, IPeerToPeerProtocol):
    ...
    class RoutePeerToPeerMessage(AbstractPeertoPeerMessage):
        ...
        def init(self, id, sourceId, targetId, ttl, priority, size, time):
            AbstractPeertoPeerMessage.init(self, id, sourceId, targetId, ttl, priority, size, time)
            if self.hasParameter("peerToPeerMessage"):
                message = self.getParameter("peerToPeerMessage")
                message.init(message.getId(), message.getSourceId(), self.getTargetId(), message.getTTL(), message.getPriority(), message.getSize(), message.getTime())
        ...
        @public    
        def setHop(self, hop):
            AbstractPeertoPeerMessage.setHop(self, hop)
            if self.hasParameter("peerToPeerMessage"):
                message = self.getParameter("peerToPeerMessage")
