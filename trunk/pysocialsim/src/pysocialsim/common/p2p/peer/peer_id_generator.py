...
class PeerIdGenerator(object):
    ...
    def generatePeerId(self, peerType):
        ...
        if peerType == IPeerToPeerNetwork.SUPER_PEER:
            id += "superpeer:id:" + sha.new(str(peerType) + str(random.random() * 1000000000000000000)).hexdigest()
        elif peerType == IPeerToPeerNetwork.SIMPLE_PEER:
            id += "simplepeer:id:" + sha.new(str(peerType) + str(random.random() * 1000000000000000000)).hexdigest()
