        ...
        if peerToPeerMessage.getHandle() == IPeerToPeerProtocol.ROUTE:
            if peerToPeerMessage.countPeerIds() < peerToPeerMessage.getTTL():
                ...
                if message.hasParameter("backTrace"):
                    newBackTrace = [] + message.getParameter("backTrace")
                    newBackTrace.append(peer.getId())
...
#                            peer.join(peerToPeerMessage.getPriority())
            ...
            if len(routes) > 0:        
                ...
                trace = route.getTrace()
                try:
                    if not peer.getNeighbor(trace[len(trace) - 1]):
            ...
            if peer.getId() == message.getFirst():
                ...
                if peer.hasNeighbor(peerId):
                    ...
                    try:
                        ...
                        messagesLogFile = open("fails.log", "a")
                        line = str(cloneMsg.getPriority()) + " " + cloneMsg.getId() + " " + cloneMsg.getHandle() + " " + cloneMsg.getSourceId() + " " + self.getPeerToPeerMessage().getTargetId() + " " + str(cloneMsg.getTime()) + " " + str(cloneMsg.getSize()) + " " + str(int(cloneMsg.getTime() + cloneMsg.getPriority())) + " " + str(cloneMsg.getHop() + 1) + " " + str(network.countConnectedPeers(IPeerToPeerNetwork.SUPER_PEER)) + " " + str(network.countConnectedPeers(IPeerToPeerNetwork.SIMPLE_PEER)) 
                        messagesLogFile.write(str(line)+"\n")
                        messagesLogFile.close()
                        peer.removeNeighbor(neighbor.getId())
                        #peer.join(message.getPriority())             
    ...
    class RoutePeerToPeerMessageHandler(AbstractPeerToPeerMessageHandler):
        ...
        def execute(self):
            ...
            if peer.getId() == message.getFirst():
                ...
                peerToPeerMessage.init(peerToPeerMessage.getId(), peerId, peer.getId(), message.getTTL(), peerToPeerMessage.getPriority(), peerToPeerMessage.getSize(), message.getTime())
                for peerId in backTrace:
                    peerToPeerMessage.registerPeerId(peerId)
