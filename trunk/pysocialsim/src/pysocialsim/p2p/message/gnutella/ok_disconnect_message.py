...
class OKDisconnectMessage(AbstractP2PMessage):
    ...
    def __init__(self, id, sourceId, targetId, ttl, priority):
        self.initialize("OK_DISCONNECT", id, sourceId, targetId, ttl, priority)
    
    @public
    def clone(self):
        message = OKDisconnectMessage(self.getId(), self.getSourceId(), self.getTargetId(), self.getTTL(), self.getPriority())
